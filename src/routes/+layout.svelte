...
<!-- <BottomNav position="fixed"  classInner="grid-cols-3 border-black border-t-[0.15rem] "  >
	...
	<a href={item.route} class="flex h-full flex-col items-center justify-center">
...
<div class="fixed bottom-0 left-0 z-50 w-full h-14 bg-white/75 backdrop-blur-md md:hidden border-t-[0.1rem] border-black dark:bg-gray-700 dark:border-gray-600">
    <div class="grid grid-cols-3 h-full max-w-lg mx-auto font-medium group-hover:bg-gray-800 ">
		...
		{/if}
            <span class="text-sm text-black dark:text-gray-400 group-hover:text-cyan-900  dark:group-hover:text-blue-500">{item.title}</span>
        </a>
